...
All other functions support "Run_plot" and are not intended to be run independently
"""
#Import packages
...
def Overall(wav, number, E_int, V_int, H, T, zb, rhs, lhs_pol): #Produces the total scattering rate and outputs overall ratios
    '''
    Internal function: Calculates the total scattering rates via both methods and outputs their ratios. Also outputs the limit of this summation process.
    '''
...
def get_Raman(final_st, E_int, freq, H, T, rhs, zb, pol):
    '''
    Internal function: For calculating a given Raman scattering rate, the same in principle as in 'Main_calculation_functions', but at a sub-state more convenient for the current purpose.
    '''
...
def Running_Disc(wav, pol, number): #For a given maximum number of discrete states, calculates the total scattering rate at a particular wavelength
    '''
    Internal function: for calculating the waveength dependent scattering rate as the sum over a specified number of discrete intermediate states as a cumulative series
    '''
    E_a = -imp.redm*(1/8)
    freq = (imp.h*imp.c)/(wav*10**(-9)*imp.Eh)
    Es = imp.Schrodinger((0, 0), nmax, k)[0]
    
    discs = Disc_Rate(1, 0, freq, E_a, pol, 1, number)+ Disc_Rate(1, 0, freq, E_a, pol, -1, number)
    for i in range(2, 6):
        if wav < (10**9*imp.c*imp.h)/((Es[i] - Es[1])*imp.Eh):
            discs = discs + Disc_Rate(i+1, 0, freq, E_a, pol, 1, number) + Disc_Rate(i+1, 2,freq, E_a, pol, 1, number)
        else:
            return discs
...
def Disc_Rate(n, l, freq, E_a, pol, sign, number):
    '''
    Internal function: calculates one scattering rate as the sum over a speicified number of discrete intermediate states in a cumulative series
    '''
...
def Limit_Disc(wav, pol, number): #For a given maximum number of discrete states, calculates the total scattering rate at a particular wavelength
    '''
    Internal function: for calculating the waveength dependent scattering rate as the sum over a specified number of discrete intermediate states as a total limit
    '''
    E_a = -imp.redm*(1/8)
    freq = (imp.h*imp.c)/(wav*10**(-9)*imp.Eh)
    Es = imp.Schrodinger((0, 0), nmax, k)[0]
    
    discs = Disc_Lim(1, 0, freq, E_a, pol, 1, number)+ Disc_Lim(1, 0, freq, E_a, pol, -1, number)
    for i in range(2, 6):
        if wav < (10**9*imp.c*imp.h)/((Es[i] - Es[1])*imp.Eh):
            discs = discs + Disc_Lim(i+1, 0, freq, E_a, pol, 1, number) + Disc_Lim(i+1, 2,freq, E_a, pol, 1, number)
        else:
            return discs
...
def Disc_Lim(n, l, freq, E_a, pol, sign, number):
    '''
    Internal function: calculates one scattering rate as the sum over a speicified number of discrete intermediate states as a total limit
    '''
    E_b = -imp.redm*(1/(2*n**2))
...
def Disc_n(nP, freq, n, l, E_a, E_b, sign):
    '''
    Internal function: calculates the discrete matrix elements that contribure to the sum
    '''
...
def NormL(n, l): # Normalising function for the wavelength
    '''
    Intenal function: Normalises the analytic wavefunciton
    '''
    return np.sqrt(math.factorial(n-l-1)/(2*n*math.factorial(n+l)))

def wavfunc(r, n, l): # Hydrogen bound state wavefunctions
    '''
    Internal function: Outputs bound state wavefunctions
    '''
...
def Run_plot():
    '''
    Function designed to poduce the plot which is the main output of this module. No inputs. The function returns a plot and numerical values for the 4 limiting cases.
    '''
    ...
    markers = ["o", "s", "x", "v"] # for longer wavelength strings, the user may have to add to this
